...
/* WhatsApp's own mark. Inlined rather than pulled from lucide, which has no
   brand glyphs, and rather than an image, so it stays crisp and needs no
   network request. */
function WhatsAppMark({ size = 26 }) {
  return (
 ...
 *
 * Appears after a short scroll rather than immediately, so it does not cover
 * the hero on a phone before anyone has read it. Sits above the cart drawer's
 * page content but below any modal.
 */
export default function WhatsAppButton() {
  ...
  const digits = String(settings?.whatsapp || settings?.phone || '+91 90122 57555').replace(/\D/g, '');
  const number = digits.length === 10 ? `91${digits}` : digits;
  const text = encodeURIComponent(
    settings?.whatsappMessage
    || 'Hi Sukoon! I would like to know more about your crystal bracelets.'
  );
